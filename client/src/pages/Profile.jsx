...
export default function Profile() {
 ...
 const fileRef = useRef(null);
  ...
  useEffect(() => {
    ...
    axios.get(`https://youtube-lite-35l5.onrender.com/api/videos?mine=true`, { headers: { Authorization: `Bearer ${token}`, } })
  ...
  const uploadImage = async (file) => {
    ...
    const res = await axios.post(`https://youtube-lite-35l5.onrender.com/api/upload/profile-image`, fd, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`https://youtube-lite-35l5.onrender.com/api/videos/${id}`, { headers: { Authorization: `Bearer ${token}` }});
